...
All above are demonstrated via various exercises in a Fabric notebook, which you can find in ths folder (called **Cosmos DB Search Exercise.ipynb**)

## How to start

You need to do below to be able to run the exercises in Fabric.

1- In your Fabric workspace, click on "Import" then "Notebook" and then choose the exercise notebook to upload.

2- Open the notebook. On the leftm under Explorer section, click on "..." near "Built-in", then click on "Upload files". Choose the PDF_RawChunks.json file to upload.
...
OPENAI_KEY=""
```
Now, all the empty values should be filled with your own info and credentials:
